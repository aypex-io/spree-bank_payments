...
RSpec.describe SpreeBankTransfer::PollJob do
  ...
  it 'records a failure and does not raise when the reconciler blows up, without wedging other payment methods' do
    ...
    allow_any_instance_of(SpreeBankTransfer::Reconcilers::Manual).to receive(:poll) do |instance, **|
  ...
  it 'does not wedge later payment methods when reconciler_state itself raises' do
    ...
    allow_any_instance_of(SpreeBankTransfer::Gateway).to receive(:reconciler_state) do |pm|
      if pm.id == payment_method.id
        raise ActiveRecord::RecordNotUnique, 'concurrent create'
      else
        SpreeBankTransfer::ReconcilerState.find_or_create_by!(payment_method_id: pm.id)
      end
    end
  ...
  it 'polls with since: derived from the last successful run minus the overlap window' do
    ...
    captured_since = nil
    allow_any_instance_of(SpreeBankTransfer::Reconcilers::Manual).to receive(:poll) do |_instance, since:|
