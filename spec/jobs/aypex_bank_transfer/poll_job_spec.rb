...
RSpec.describe AypexBankTransfer::PollJob do
  ...
  it 'does not wedge later payment methods when reconciler_state itself raises' do
    ...
    allow_any_instance_of(AypexBankTransfer::Gateway).to receive(:reconciler_state) do |pm|
      if pm.id == payment_method.id
        raise ActiveRecord::RecordNotUnique, 'concurrent create'
      else
        AypexBankTransfer::ReconcilerState.find_or_create_by!(payment_method_id: pm.id)
      end
    end
