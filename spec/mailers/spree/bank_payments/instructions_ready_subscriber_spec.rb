...
RSpec.describe 'Spree::BankPayments default mailer event subscription', type: :mailer do
  ...
  it 'enqueues the instructions mail when bank_transfer.instructions_ready is published and the default mailer is enabled' do
    expect(Spree::BankPayments::Config.disable_default_mailer).to be(false)

    expect do
      payment_method.create_payment_session(order: order)
    end.to have_enqueued_mail(Spree::BankPayments::InstructionsMailer, :instructions)
  end
  ...
  it 'enqueues the reminder mail when bank_transfer.reminder_due is published and the default mailer is enabled' do
    ...
    end.to have_enqueued_mail(Spree::BankPayments::InstructionsMailer, :reminder)
  end

  describe 'registry survives a Zeitwerk reload' do
    # Same mechanism, and same regression, as
    # spec/models/spree/bank_payments/reconcilers/manual_spec.rb: spree_core's
    ...
    # config/initializers/spree.rb) means Spree::BankPayments.
    ...
    it 'does not stack duplicate subscriptions across repeated reloads' do
      ...
      expect do
        payment_method.create_payment_session(order: order)
      end.to have_enqueued_mail(Spree::BankPayments::InstructionsMailer, :instructions).once
    end
  end
  ...
  describe 'when the default mailer is disabled' do
    ...
    it 'does not (re-)subscribe the default mailer after a reload with the registry cleared' do
      ...
      # calls on every real reload (see lib/spree/bank_payments/subscribers.rb)
      ...
      begin
        ...
        expect do
          payment_method.create_payment_session(order: order)
        end.not_to have_enqueued_mail(Spree::BankPayments::InstructionsMailer, :instructions)
      ensure
        Spree::BankPayments::Config.disable_default_mailer = false
        Rails.application.reloader.prepare!
      end
    end
  end
