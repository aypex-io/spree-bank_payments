...
RSpec.describe 'SpreeBankPayments default mailer event subscription', type: :mailer do
  ...
  it 'enqueues the instructions mail when bank_transfer.instructions_ready is published and the default mailer is enabled' do
    expect(SpreeBankPayments::Config.disable_default_mailer).to be(false)

    expect do
      payment_method.create_payment_session(order: order)
    end.to have_enqueued_mail(SpreeBankPayments::InstructionsMailer, :instructions)
  end
  ...
  it 'enqueues the reminder mail when bank_transfer.reminder_due is published and the default mailer is enabled' do
    ...
    end.to have_enqueued_mail(SpreeBankPayments::InstructionsMailer, :reminder)
  end

  describe 'registry survives a Zeitwerk reload' do
    # Same mechanism, and same regression, as
    # spec/models/spree_bank_payments/reconcilers/manual_spec.rb: spree_core's
    ...
    # config/initializers/spree.rb) means SpreeBankPayments.
    ...
    it 'does not stack duplicate subscriptions across repeated reloads' do
      ...
      expect do
        payment_method.create_payment_session(order: order)
      end.to have_enqueued_mail(SpreeBankPayments::InstructionsMailer, :instructions).once
    end
  end
  ...
  describe 'when the default mailer is disabled' do
    ...
    it 'does not (re-)subscribe the default mailer after a reload with the registry cleared' do
      ...
      # calls on every real reload (see lib/spree_bank_payments/subscribers.rb)
      ...
      begin
        ...
        expect do
          payment_method.create_payment_session(order: order)
        end.not_to have_enqueued_mail(SpreeBankPayments::InstructionsMailer, :instructions)
      ensure
        SpreeBankPayments::Config.disable_default_mailer = false
        Rails.application.reloader.prepare!
      end
    end
  end
