...
RSpec.describe AypexBankTransfer::Reconcilers::Manual do
  ...
  describe 'registry survives a Zeitwerk reload' do
    ...
    it 're-registers manual when to_prepare re-runs after the registry is cleared' do
      expect(AypexBankTransfer::Reconcilers::Base.registry).to include('manual')

      AypexBankTransfer::Reconcilers::Base.registry.clear
      expect(AypexBankTransfer::Reconcilers::Base.registry).to eq({})

      Rails.application.reloader.prepare!

      expect(AypexBankTransfer::Reconcilers::Base.build(payment_method: payment_method))
