...
RSpec.describe SpreeBankTransfer::Reconcilers::Manual do
  ...
  describe 'registry survives a Zeitwerk reload' do
    ...
    it 're-registers manual when to_prepare re-runs after the registry is cleared' do
      expect(SpreeBankTransfer::Reconcilers::Base.registry).to include('manual')

      SpreeBankTransfer::Reconcilers::Base.registry.clear
      expect(SpreeBankTransfer::Reconcilers::Base.registry).to eq({})

      Rails.application.reloader.prepare!

      expect(SpreeBankTransfer::Reconcilers::Base.build(payment_method: payment_method))
