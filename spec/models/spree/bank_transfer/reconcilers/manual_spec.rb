...
RSpec.describe Spree::BankTransfer::Reconcilers::Manual do
  ...
  describe 'registry survives a Zeitwerk reload' do
    ...
    it 're-registers manual when to_prepare re-runs after the registry is cleared' do
      expect(Spree::BankTransfer::Reconcilers::Base.registry).to include('manual')

      Spree::BankTransfer::Reconcilers::Base.registry.clear
      expect(Spree::BankTransfer::Reconcilers::Base.registry).to eq({})

      Rails.application.reloader.prepare!

      expect(Spree::BankTransfer::Reconcilers::Base.build(payment_method: payment_method))
