...
RSpec.describe Spree::BankPayments::Reconcilers::Manual do
  ...
  describe 'registry survives a Zeitwerk reload' do
    ...
    it 're-registers manual when to_prepare re-runs after the registry is cleared' do
      expect(Spree::BankPayments::Reconcilers::Base.registry).to include('manual')

      Spree::BankPayments::Reconcilers::Base.registry.clear
      expect(Spree::BankPayments::Reconcilers::Base.registry).to eq({})

      Rails.application.reloader.prepare!

      expect(Spree::BankPayments::Reconcilers::Base.build(payment_method: payment_method))
